...
1. A **service connection type** named `Infisical` (`ms.vss-endpoint.service-endpoint-type`). It exposes a URL field plus two auth schemes:
   ...
   - `OIDC` — `ms.vss-endpoint.endpoint-auth-scheme-none`. The auth module supports OIDC end-to-end (`src/auth/oidc.ts`), but the task does **not** yet acquire an Azure DevOps OIDC JWT; only Universal Auth is wired through `getAccessToken`.
